...
from caffe2pytorch.prototxt import *
import caffe
import caffe.proto.caffe_pb2 as caffe_pb2
import caffe2pytorch.SpatialCrossMapLRN as SpatialCrossMapLRNOld
from itertools import product as product
from caffe2pytorch.detection import Detection, MultiBoxLoss
...
class Reshape(nn.Module):
    ...
    def forward(self, x):
        ...
        new_dims = [orig_dims[i] if self.dims[i] == 0 else self.dims[i] for i in
                    range(len(self.dims))]
...
class PriorBox(nn.Module):
    ...
    def __repr__(self):
        return 'PriorBox(min_size=%f, max_size=%f, clip=%d, step=%d, offset=%f, variances=%s)' % (
        self.min_size, self.max_size, self.clip, self.step, self.offset, self.variances)
...
class CaffeNet(nn.Module):
    def __init__(self, protofile, width=None, height=None, channels=None, omit_data_layer=False,
                 phase='TRAIN'):
    ...
    def load_weights(self, caffemodel):
        if self.has_mean:
            ...
            mu = mu.mean(1).mean(1)
            mean_img = torch.from_numpy(mu).view(channels, 1, 1).expand(channels, height,
                                                                        width).float()
        ...
        while i < layer_num:
            ...
            if ltype in ['Convolution', 'Deconvolution']:
                ...
                bias = True
                if convolution_param.has_key('bias_term') and convolution_param[
                    'bias_term'] == 'false':
                    bias = False
                ...
                print('load weights %s' % lname)
                self.models[lname].running_mean.copy_(torch.from_numpy(
                    np.array(lmap[lname].blobs[0].data) / lmap[lname].blobs[2].data[0]))
                self.models[lname].running_var.copy_(torch.from_numpy(
                    np.array(lmap[lname].blobs[1].data) / lmap[lname].blobs[2].data[0]))
                i = i + 1
            elif ltype == 'Scale':
                print('load weights %s' % lname)
    ...
    def create_network(self, net_info, input_width=None, input_height=None, input_channels=None):
        ...
        while i < layer_num:
            ...
            if ltype in ['Data', 'AnnotatedData']:
                if not self.omit_data_layer:
                    ...
                    blob_height[data_name] = data.size(
                        2)  # int(layer['transform_param']['resize_param']['height'])
                    blob_width[data_name] = data.size(
                        3)  # int(layer['transform_param']['resize_param']['width'])
            ...
            if ltype == 'Convolution':
                ...
                stride = int(convolution_param['stride']) if convolution_param.has_key(
                    'stride') else 1
                ...
                if convolution_param.has_key('bias_term') and convolution_param[
                    'bias_term'] == 'false':
                    bias = False
                models[lname] = nn.Conv2d(channels, out_filters, kernel_size=kernel_size,
                ...
                momentum = 0.9
                if layer.has_key('batch_norm_param') and layer['batch_norm_param'].has_key(
                ...
                blob_width[tname] = int(
                    math.ceil((blob_width[bname] + 2 * padding - kernel_size) / float(stride))) + 1
                blob_height[tname] = int(
                    math.ceil((blob_height[bname] + 2 * padding - kernel_size) / float(stride))) + 1
                blob_channels[tname] = blob_channels[bname]
                i = i + 1
            elif ltype == 'Eltwise':
                operation = 'SUM'
                ...
                models[lname] = PriorBox(min_size, max_size, aspects, clip, flip, step, offset,
                                         variances)
                ...
                models[lname] = MultiBoxLoss(num_classes, overlap_threshold, prior_for_matching,
                ...
                models[lname] = nn.ConvTranspose2d(in_channels, out_channels,
            ...
            input_channels = blob_channels[bname] if type(bname) != list else blob_channels[
                bname[0]]
            ...
            output_channels = blob_channels[tname] if type(tname) != list else blob_channels[
                tname[0]]
            print('create %-20s (%4d x %4d x %4d) -> (%4d x %4d x %4d)' % (
            lname, input_channels, input_height, input_width, output_channels, output_height,
            output_width))
