...
class H36m(data.Dataset):
    def __init__(self, cfg, img_path, train=True):
        ...
        if not cfg.load_pickle:
            ...
            pickle.dump(
                self.annot_imgs_path,
                open(os.path.join("data/pickle", "annot_imgs_path.pickle"), 'wb')
            )
            pickle.dump(
                self.annot_Y2d,
                open(os.path.join("data/pickle", "annot_Y2d.pickle"), 'wb')
            )
            pickle.dump(
                self.annot_Y3d,
                open(os.path.join("data/pickle", "annot_Y3d.pickle"), 'wb')
            )
            pickle.dump(
                self.annot_bbox,
                open(os.path.join("data/pickle", "annot_bbox.pickle"), 'wb')
            )
            pickle.dump(
                self.annot_num_img,
                open(os.path.join("data/pickle", "annot_num_img.pickle"), 'wb')
            )

        else:
            self.annot_imgs_path = pickle.load(
                open(os.path.join("data/pickle", "annot_imgs_path.pickle"), 'rb')
            )
            self.annot_Y2d = pickle.load(
                open(os.path.join("data/pickle", "annot_Y2d.pickle"), 'rb')
            )
            self.annot_Y3d = pickle.load(
                open(os.path.join("data/pickle", "annot_Y3d.pickle"), 'rb')
            )
            self.annot_bbox = pickle.load(
                open(os.path.join("data/pickle", "annot_bbox.pickle"), 'rb')
            )
            self.annot_num_img = pickle.load(
                open(os.path.join("data/pickle", "annot_num_img.pickle"), 'rb')
            )
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join('data/images',img_dir1, img_name)
        ...
        # if self.is_train:
        target = np.zeros((self.num_class, self.out_res[0], self.out_res[1]))
